...
async function handleSubmit(e) {
    ...
    if (uploadedImages.length > 0) {
        addImageMessage(uploadedImages, 'user');
    }
...
// 이미지 메시지 추가
function addImageMessage(images, sender) {
    console.log(`이미지 메시지 추가 (${sender}):`, images.length, '개');

    const messageDiv = document.createElement('div');
    messageDiv.className = `message ${sender}`;

    // 이미지들을 그리드로 표시
    const imagesHtml = images.map((imageSrc, index) => `
        <img src="${imageSrc}" alt="Uploaded image ${index + 1}" style="max-width: 300px; max-height: 300px; margin: 5px; border-radius: 8px; object-fit: cover;">
    `).join('');
